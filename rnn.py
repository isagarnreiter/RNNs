...
dale_network_params['autapses'] = False
...
# pre_connect = np.zeros((nb_excn* nb_excn))
# pre_connect[:nb_excn*2] = 1
# np.random.shuffle(pre_connect)
# rand_connect1 = np.array(np.split(pre_connect, indices_or_sections = nb_excn))
# np.random.shuffle(pre_connect)
# rand_connect2 = np.array(np.split(pre_connect, indices_or_sections = nb_excn))

# rec_connectivity[nb_excn:nb_excn*2,:nb_excn] = rand_connect1
# rec_connectivity[:nb_excn,nb_excn:nb_excn*2] = rand_connect2
# rec_connectivity[N_rec-nb_inhn:N_rec, :nb_excn] = 0
# rec_connectivity[nb_excn*2:N_rec-nb_inhn, nb_excn:nb_excn*2] = 0
...
ax1.plot(range(0, len(x[0,:,:])*dt,dt), x[trial_nb,:,:3])
ax1.set_title("Input")
ax1.legend(["Output Channel 1", "Output Channel 2", 'go cue'])

ax2 = plt.subplot(2,1,2)
ax2.plot(range(0, len(x[0,:,:])*dt,dt), x[trial_nb,:,3], c = 'grey')
