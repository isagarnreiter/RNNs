...
class PerceptualDiscrimination(Task):
    ...
    def generate_trial_params(self, batch, trial):
        ...
        params['onset_time'] = np.random.random()*self.T*(1/8)
        params['stim_duration'] = self.T*(1/4)+np.random.random()*self.T*(1/8)
        params['go_cue_onset'] = params['onset_time']+params['stim_duration']+ self.T/8 +np.random.random()*self.T*(1/4)
